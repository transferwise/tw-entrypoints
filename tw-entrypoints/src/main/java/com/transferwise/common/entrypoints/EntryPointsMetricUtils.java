...
public class EntryPointsMetricUtils {
  ...
  public static DistributionSummary summaryWithoutBuckets(MeterRegistry meterRegistry, String name, Iterable<Tag> tags) {
    return DistributionSummary.builder(name).tags(tags)
        ...
        .maximumExpectedValue(1L) // TODO: this limits number on buckets and is currently needed because the above histogram disabling does not
        // work due to many services using management.metrics.distribution.percentiles-histogram.all = true.
  ...
  public static Timer timerWithoutBuckets(MeterRegistry meterRegistry, String name, Iterable<Tag> tags) {
    return Timer.builder(name).tags(tags)
        ...
        .maximumExpectedValue(Duration.ofNanos(1L)) // TODO: last two are limiting number on buckets and are currently needed because the
        // above histogram disabling does not work due to many services using management.metrics.distribution.percentiles-histogram.all = true.
